...
(function ($, Drupal) {
    ...
    function handleSortDropdown() {
        ...
        if (searchParams.has('a[0][f]') && searchParams.has('a[0][i]') && searchParams.has('a[0][v]')) {
            // TODO: CHECK if search mode is on, add search_api_relevence to sort dropdown
            if (0 == $('.pager__sort select option[value="search_api_relevance_asc"]').length ) {
                jQuery(".pager__sort select").prepend('<option value="search_api_relevance_asc">Relevance ↑</option>');
            }
            if (0 == $('.pager__sort select option[value="search_api_relevance_desc"]').length ) {
                jQuery(".pager__sort select").prepend('<option value="search_api_relevance_desc" selected="selected">Relevance ↓</option>');
            }
        }
        else {
            // if not, remove search_api_relevence from sort dropdown
            jQuery('.pager__sort select option[value="search_api_relevance_asc"]').remove();
            jQuery('.pager__sort select option[value="search_api_relevance_desc"]').remove();
        }
    }
